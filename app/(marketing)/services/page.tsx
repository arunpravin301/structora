      ...
      <section className="py-[118px] bg-mist"><Container>
        ...
        <Reveal><table className="w-full border-collapse border border-line">
          <thead><tr>{["Package","Per sq ft","What is included"].map(h=>(
            <th key={h} className="border border-line p-4.5 text-left bg-ink text-white font-fr font-semibold text-[19px]" style={{padding:18}}>{h}</th>))}</tr></thead>
          <tbody>
            {[["Basic","₹2,400 / sq ft","Structure, brickwork and plastering, to a ready-to-finish standard. (Inclusions draft, to confirm.)"],
              ...
              <tr key={p}>
                <th className="border border-line p-4.5 text-left font-fr font-semibold text-[17px] text-navy bg-mist" style={{padding:18}}>{p}</th>
                <td className="border border-line p-4.5 font-fr text-[22px] text-ink" style={{padding:18}}>{pr}</td>
