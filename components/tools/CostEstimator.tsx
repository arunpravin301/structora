...
export default function CostEstimator() {
  ...
  return (
    <div className="border border-line p-0 bg-white flex flex-col md:flex-row items-stretch">
          ...
          <div>
            ...
            <div className="relative">
              <select 
                ...
                className="w-full appearance-none bg-slate/5 border-b border-line px-4 py-3 text-[15px] focus:outline-none focus:border-brand transition-colors font-inter cursor-pointer rounded-t-sm"
              >
                {site.offices.map(o => (
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {Object.keys(site.packageMultiplier).map(p => (
                <button key={p} onClick={() => setPkg(p)} className={`px-4 py-2 text-sm border transition-colors ${pkg === p ? 'border-brand bg-brand text-white' : 'border-line text-slate hover:border-brand'}`}>{p}</button>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="w-full md:w-[320px] bg-slate-900 text-white relative flex flex-col justify-center p-10 border-l border-line/10 overflow-hidden">
        {state === "idle" && (
          <div className="absolute inset-0 z-20 bg-slate-900 flex flex-col justify-center p-8 text-center">
        ...
        {state === "locked" && (
          <div className="absolute inset-0 z-10 bg-slate-900/80 backdrop-blur-md flex flex-col justify-center p-8 text-center transition-all duration-500">
            ...
            <h3 className="font-outfit text-lg font-semibold mb-2">Unlock estimate</h3>
            ...
            <form onSubmit={unlock} className="space-y-3 text-left">
              ...
              <button disabled={busy} className="w-full bg-white text-slate-900 font-semibold text-[13px] py-3 mt-3 transition-transform active:scale-[0.98]">{busy ? "Unlocking..." : "Reveal Price"}</button>
            </form>
          </div>
        )}
        ...
        <div className={`transition-all duration-700 ${state !== "unlocked" ? 'opacity-20 blur-md scale-95' : 'opacity-100 blur-0 scale-100'}`}>
          ...
          <div className="font-outfit font-semibold text-[32px] leading-tight text-white mb-2">{priceRange}</div>
