    ...
    <footer className="bg-[#06101C] text-slatel pt-20 pb-7 text-sm">
      <Container>
        <div className="grid grid-cols-[1.7fr_1fr_1fr_1fr] max-[980px]:grid-cols-2 max-[600px]:grid-cols-1 gap-9 pb-11 border-b border-lined">
          <div>
            <div className="font-fr font-semibold text-2xl tracking-[.05em] text-white mb-4">STRUCTORA</div>
          ...
          <div><h4 className="text-white text-xs font-medium tracking-[.1em] uppercase mb-4 font-gs">Navigate</h4>
            {site.nav.map((n) => <Link key={n.href} className="block py-1.5 hover:text-white" href={n.href}>{n.label}</Link>)}
          </div>
          <div><h4 className="text-white text-xs font-medium tracking-[.1em] uppercase mb-4 font-gs">Offices</h4>
            {site.contact.addresses.map((o) => <span key={o.name} className="block py-1.5">{o.name}</span>)}
          </div>
          <div><h4 className="text-white text-xs font-medium tracking-[.1em] uppercase mb-4 font-gs">Connect</h4>
